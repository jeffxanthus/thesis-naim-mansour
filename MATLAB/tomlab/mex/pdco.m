...
% 23 Oct 2002: HKH: Fname and Aname can now be function handles.
...
% 11 Feb 2005: Fname and Aname can be arrays or strings but not
%              function handles.  If necessary, use func2str;
%              for example, Fname = func2str(@phi).
% 02 Aug 2006: HKH - Adaption of 2005 changes to Tomlab
%-----------------------------------------------------------------------
...
if PriLev > 0
  ...
  maxf      = 20;     % Linesearch backtrack limit (function evaluations)
...
% mufirst = mu0*(x0min * z0min);
  ...
  mu      = mufirst; [cL,cU,center,Cinf,Cinf0] = ...
...
if PriLev > 0
 ...
 %  stepx  = min( [stepx1,   stepx2,   stepxL,   stepxU] );
    ...
    %-------------------------------------------------------------------
if PriLev > 0
    str1    = sprintf('\n%3g%6.2f' , PDitns      , log10(mu)   );
    str2    = sprintf('%9.3f%9.3f' , stepx       , stepz       );
    if stepx < 0.001 | stepz < 0.001
      str2 = sprintf('%9.1e%9.1e' , stepx       , stepz       );
    end
