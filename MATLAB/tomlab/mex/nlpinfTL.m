% TOMLAB NLPINF SQP Maximum-Norm Solver
%
...
%
% PrintFile Name of NLPINF Print file.  Amount and type of printing 
...
% Solver   Name of the solver (NLPINF).
...
% Written Jul 8, 2009.    Last modified Jul 8, 2009.
...
if ~isfield(Prob,'NLPINF')  % Make sure NLPINF field exists
   Prob.NLPJOB = [];
end

Result=ResultDef(Prob);     % Define the Result struct
Result.Solver='NLPINF';     % Solver name
...
NLPINF   = DefPar(Prob,'NLPINF',[]);
acc       = DefPar(NLPINF,'acc',1e-14);
accqp     = DefPar(NLPINF,'accqp',1e-14);
ressize   = DefPar(NLPINF,'ressize',0.0);
maxfun    = DefPar(NLPINF,'maxfun',20);
maxit     = DefPar(NLPINF,'maxit',100);
maxnm     = DefPar(NLPINF,'maxnm',0);
tolnm     = DefPar(NLPINF,'tolnm',0.0);
options   = [acc, accqp, ressize, maxfun, maxit, maxnm, tolnm];
PriLevOpt = DefPar(Prob,'PriLevOpt',0);
PrintFile = DefPar(NLPINF,'PrintFile','nlpinf.txt');

[x, Inform, Iter, cLamda, Obj, f_eval, g_eval] = nlpinf ( ...
...
% MODIFICATION LOG:
%
% 090708 bjo Created NLPINF interface.
