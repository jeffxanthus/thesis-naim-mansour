...
% with convex or nonconvex sub problems using NLP relaxation (Formulated as minlp-IP)
%
...
%
% MaxCPU:   Maximal CPU Time (in seconds) to be used by minlpSolve,
...
%
% WarmStart If true, >0, minlpSolve reads the output from the last
%           run from Prob.minlpSolve, if it exists. If it doesn't exist, minlpSolve attempts
%           to open and read warm start data from mat-file minlpSolveSave.mat.
%           minlpSolve uses the warm start information to continue from the last run.
%           The mat-file minlpSolveSave.mat is saved every Prob.MIP.SaveFreq iteration
%
...
%
% SaveFreq  Warm start info saved on minlpSolveSave.mat every SaveFreq iteration
...
% structure, make a call to WarmDefGLOBAL before calling minlpSolve.
...
% Written Mar 31, 2004.   Last modified June 5, 2010.

function ResultMIP = minlpSolve(Prob)
...
elseif any(IntVars==0) | all(IntVars==1)
   ...
   if any(IntVars < 1 | IntVars > n)
      error('minlpSolve: Illegal IntVars vector');
   end
   IV(IntVars)=1;
end
% RealVars = find(~IV);
...
ResultMIP.Solver          = 'minlpSolve';
ResultMIP.Prob            = Prob;
ResultMIP.SolverAlgorithm = 'Branch & Bound MINLP';
ResultMIP.minlpSolve      = [];
...
% minlpSolve warm start parameters
MINLP = DefPar(Prob,'minlpSolve',[]);
...
if WarmStart == 1 
   ...
   WarmStart            = 0;
   if SaveFreq >= 0
      if exist('minlpSolveSave.mat','file')
         delete('minlpSolveSave.mat');
      end
   end
...
while ~(Iter >= MaxIter)
   if isempty(L)
      ...
      if fIPMin == Inf % Empty feasible set
         ...
         if IterPrint
            ...
            fprintf('--- minlpSolve NLP Branch & Bound converged! ')
   ...
   elseif GapOK == 1
      ...
      if IterPrint
         PrintIter(3,Iter,[],[],fIPMin,fIPIter,fLB,L,0,0,x,[],[],...
                   Inform,ExitFlag,FuEv,BigNum,'');
         fprintf('\n--- minlpSolve: User defined duality gap reached. ');
         fprintf('Its(nodes visited) = %d. ',Iter)
         fprintf('Total NLP Its =%d. ',SumMinIt)
         fprintf('Optimal f(x) =%22.16f.',fIPMin);
         fprintf('\n');
   ...
   if i == 1 & WarmStart == 0     % 1st step
      ...
      if CONVEX
         Result = tomSolve(SolverNLP,ProbNLP);
	 if PriLev > 2
   ...
   if fNode <  fIPMin
      ...
      if IntVar
	    ...
	    % The following seems meaningless as it stands
            % if abs(fIPMin - nlp_f(xIPMin,ProbNLP)) < 1E-10 %CHANGED
            %    ENDTREE=1; 
            % else
            %    xC=[]; % Will always be empty at this point
            % end
         end
         ENDTREE=1;
      else
   ...
   if ~isinf(fIPMin)
      ...
      % Check duality gap is low enough
      if Gap <= DualGap
         GapOK = 1;
      else
         GapOK = 0;
   ...
   if SaveFreq > 0 & mod(Iter,SaveFreq) == 0
      ...
      try
         ...
         save('minlpSolveSave.mat','Name','xIPMin','fIPMin','fIPIter','vIPMin',...
                            'FuncEv','GradEv','HessEv', 'ConstrEv', ...
                            'XX','HS','nS', ...
                            'L','NODE','TotIter', 'f_min','pred', 'Problem',...
                            'Depth','Icomp',...
         ...
         warning('Failed to save warm start information to minlpSolveSave.mat');
         err = lasterror;
         disp(err.message);
         disp('Warm start information is available in Result.minlpSolve');
      end
   end
   if wait 
...
ResultMIP.minlpSolve.Name      = Name;
ResultMIP.minlpSolve.xIPMin    = xIPMin;
ResultMIP.minlpSolve.fIPMin    = fIPMin;
ResultMIP.minlpSolve.fIPIter   = fIPIter;
ResultMIP.minlpSolve.vIPMin    = vIPMin;
ResultMIP.minlpSolve.FuncEv    = FuncEv;
ResultMIP.minlpSolve.GradEv    = GradEv;
ResultMIP.minlpSolve.HessEv    = HessEv;
ResultMIP.minlpSolve.ConstrEv  = ConstrEv;
ResultMIP.minlpSolve.XX        = XX(:,1:NODE);
if SOL > 0
   ResultMIP.minlpSolve.HS     = HS(:,1:NODE);
   ResultMIP.minlpSolve.nS     = nS(1,1:NODE);
else
   ResultMIP.minlpSolve.HS     = HS;
   ResultMIP.minlpSolve.nS     = nS;
end
ResultMIP.minlpSolve.L         = L;
ResultMIP.minlpSolve.NODE      = NODE;

TotIter                        = SumIter+Iter;

ResultMIP.minlpSolve.TotIter   = TotIter;
ResultMIP.minlpSolve.f_min     = f_min(1:NODE);
ResultMIP.minlpSolve.pred      = pred(1:NODE);
ResultMIP.minlpSolve.Problem.xVar   = Problem.xVar(1:NODE);
ResultMIP.minlpSolve.Problem.Bound  = Problem.Bound(1:NODE);
ResultMIP.minlpSolve.Problem.xBound = Problem.xBound(1:NODE);
ResultMIP.minlpSolve.Depth     = Depth;
ResultMIP.minlpSolve.Icomp     = Icomp;
ResultMIP.minlpSolve.Gap       = Gap;
ResultMIP.minlpSolve.SumMinIt  = SumMinIt;
RandState                      = rand('state');
ResultMIP.minlpSolve.RandState = RandState;
time                           = fix(clock);
ResultMIP.minlpSolve.time      = time;
...
% 041023  hkh  KNAPSACK removed, safe guard input for fIP, xIP, VarWeight
% 041023  hkh  Set PrintLM=0, no computation of Lagrange multipliers
% 050415  hkh  Revised, implementing a standard B&B MINLP
% 050415  hkh  Adding a loop of retries with random initial points
% 060814  med  FUNCS used for callbacks instead
% 061117  hkh  Use Prob.f_Low instead of optParam.eps_absf
% 070222  hkh  Revise IntVars handling, use new format
% 090826  hkh  cErrCompute now computes max relative error, not absolute
% 090826  hkh  Use multiMin for ~CONVEX, single local solver call for CONVEX
% 090827  hkh  Major revision
% 091002  hkh  Add Warm Start and time stamp in iteration print
% 091003  hkh  RandState added, initialize random numbers in multiMin (if Convex == 0)
% 091003  hkh  RandState saved in Result.minlpSolve, giving identical runs if WarmStart.
% 091004  hkh  Use Prob, not ProbNLP, in endSolve
% 091005  hkh  Compress arrays in Result.minlpSolve to length NODE
% 091007  hkh  Add printout of length(L) if IterPrint
% 091007  hkh  Handle fIP, without xIP, cutting nodes in tree search
% 091019  hkh  Change multiMin fields, multiMin.Info for counters etc
% 091020  hkh  Avoid catch ME, use lasterror
% 091020  hkh  Must set ProbNLP.WarmStart = 0 to avoid warm start in subsolver
...
% 100605  hkh  Bug in mod() if neg 2nd arg, SaveFreq > 0 needed before save
