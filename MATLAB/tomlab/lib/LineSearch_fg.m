...
%
% Special version when both f and g (or r and J) is computed in the same call
%
% function Result = LineSearch_fg( fg, x, p, f_0, g_0, LineParam, alphaMax,...
%                               pType, PriLev, Prob, varargin);
%
% INPUT:
%
% fg    Name of routine to compute both function value and gradient
% x     Current iterate x
% p     Search step p
...
% Copyright (c) 1994-2011 by Tomlab Optimization Inc., $Release: 7.8.0$
% Written April 17, 1994. Last modified July 24, 2011.

function Result = LineSearch( fg, x, p, f_0, g_0, LineParam, alphaMax,...
                              pType, PriLev, Prob, varargin)
...
if isempty(f_0) 
   [f_0, g_0, Result] = evalfg( fg, x, fgType, Result, Prob, varargin{:});
   Result.f_alpha = f_0;
   Result.g_alpha = g_0;
end
...
f_alpha_km1 = f_0;
g_alpha_km1 = g_0;
fp_0        = g_0'*p;	% derivative in x in search direction, fp(x) =  g(x)' * p
...
while 1
   ...
   [f_alpha_k, g_alpha_k, Result] = evalfg( fg, xEval, fgType, Result, Prob, varargin{:});
...
%                      k       k-1         k
   if f_alpha_k >=f_alpha_km1 | f_alpha_k > f_0+alpha_k*rho*fp_0 % Test 2.5.1 (rho-line)
...
%
   if abs(fp_alpha_k) <= -sigma*fp_0 | (fp_alpha_k < 0 & alpha_k==my)
      if PriLev > 0
...
%
% ============================================================
function [fgType] = CheckFunc( fg, pType)
% ============================================================
if strcmp(fg,'nlp_fg')
   fgType=1;
elseif strcmp(fg,'ls_rJ')
   fgType=2;
elseif strcmp(fg,'nlp_rJ')
   fgType=3;
elseif pType ==0
   fgType=4;
elseif pType ==1
...
% ============================================================
function [f_k, g_k, Result] = evalfg( fg, x, fgType, Result, Prob, varargin)
% ============================================================

switch fgType
case 1
   [Mode, f_k,g_k]  = nlp_fg(x, Prob, Prob.Mode, Prob.nState,varargin{:}); 
   Result.Mode      = Mode;
case 2
   [Mode, r_k, J_k] = ls_rJ(x, Prob, Prob.Mode, Prob.nState, varargin{:}); 
   f_k              = 0.5 * (r_k' * r_k);
   g_k              = J_k' * r_k;
   Result.r_k       = r_k;
   Result.J_k       = J_k;
case 3
   [Mode, r_k, J_k] = nlp_rJ(x, Prob, Prob.Mode, Prob.nState, varargin{:}); 
   f_k              = 0.5 * (r_k' * r_k);
   g_k              = J_k' * r_k;
   Result.r_k       = r_k;
   Result.J_k       = J_k;
case 4
   [f_k g_k]        = feval(fg, x, Result, Prob, varargin{:}); 
case 5
   [r_k, J_k]       = feval(fg, x, Prob, varargin{:}); 
   f_k              = 0.5 * (r_k' * r_k);
   g_k              = J_k' * r_k;
   Result.r_k       = r_k;
   Result.J_k       = J_k;
case 6
   [r_k, J_k]       = hcnn_rJ(x, Prob); 
   f_k              = 0.5 * (r_k' * r_k);
   g_k              = J_k' * r_k;
   Result.r_k       = r_k;
   Result.J_k       = J_k;
...
% MODIFICATION LOG:
%
% 110724  hkh  Created from Linesearch
