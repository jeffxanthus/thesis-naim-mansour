% TOMLAB NLPMMX SQP Min-Max Solver
%
% function Result = nlpmmxTL(Prob)
%
% INPUT:
%
...
%
% PrintFile Name of NLPMMX Print file.  Amount and type of printing 
...
% Solver   Name of the solver (NLPMMX).
...
% Written Jul 26, 2009.    Last modified Jul 26, 2009.
...
if ~isfield(Prob,'NLPMMX')  % Make sure NLPMMX field exists
   Prob.NLPJOB = [];
end

Result=ResultDef(Prob);     % Define the Result struct
Result.Solver='NLPMMX';     % Solver name
...
NLPMMX   = DefPar(Prob,'NLPMMX',[]);
acc       = DefPar(NLPMMX,'acc',1e-14);
accqp     = DefPar(NLPMMX,'accqp',1e-14);
ressize   = DefPar(NLPMMX,'ressize',0.0);
maxfun    = DefPar(NLPMMX,'maxfun',20);
maxit     = DefPar(NLPMMX,'maxit',100);
maxnm     = DefPar(NLPMMX,'maxnm',0);
tolnm     = DefPar(NLPMMX,'tolnm',0.0);
options   = [acc, accqp, ressize, maxfun, maxit, maxnm, tolnm];
PriLevOpt = DefPar(Prob,'PriLevOpt',0);
PrintFile = DefPar(NLPMMX,'PrintFile','nlpmmx.txt');
...
% MODIFICATION LOG:
%
% 090726 bjo Created NLPMMX interface.
