% TOMLAB NLPL1 SQP L1-Norm Solver
%
% function Result = nlpinfTL(Prob)
%
% INPUT:
%
...
%
% -----------------------------------------------
% Fields used in Prob.NLPL1:
% -----------------------------------------------
%
% ressize   The user must indicate a guess for the approximate size of the
%           objective funtion.  RESSIZE must not be negative.
%
% maxfun    The integer variable defines an upper bound for the number of 
%           function calls during the line search.
%
...
%
% PrintFile Name of NLPL1 Print file.  Amount and type of printing 
...
% Solver   Name of the solver (NLPL1).
% SolverAlgorithm  Description of the solver.
%
% NLPL1.u    Contains the multipliers with respect to the actual iterate
%             stored in x_k.  See user's guide.

% Kenneth Holmstrom, Tomlab Optimization Inc, E-mail: tomlab@tomopt.com
% Copyright (c) 2009 by Tomlab Optimization Inc., $Revision: 1.1 $
% Written Jul 8, 2009.    Last modified Aug 19, 2009.

function Result = nlpl1TL(Prob)
...
if ~isfield(Prob,'NLPL1')  % Make sure NLPL1 field exists
   Prob.NLPJOB = [];
end

Result=ResultDef(Prob);     % Define the Result struct
Result.Solver='NLPL1';     % Solver name
Result.SolverAlgorithm='Dense SQP L1-Norm';
BIG=1E12;
...
NLPL1   = DefPar(Prob,'NLPL1',[]);
acc       = DefPar(NLPL1,'acc',1e-14);
accqp     = DefPar(NLPL1,'accqp',1e-14);
ressize   = DefPar(NLPL1,'ressize',0.0);
maxfun    = DefPar(NLPL1,'maxfun',20);
maxit     = DefPar(NLPL1,'maxit',100);
maxnm     = DefPar(NLPL1,'maxnm',0);
tolnm     = DefPar(NLPL1,'tolnm',0.0);
options   = [acc, accqp, ressize, maxfun, maxit, maxnm, tolnm];
PriLevOpt = DefPar(Prob,'PriLevOpt',0);
PrintFile = DefPar(NLPL1,'PrintFile','nlpl1.txt');

[x, Inform, Iter, cLamda, Obj, f_eval, g_eval] = nlpl1 ( ...
...
switch(Inform)
    ...
    case 11
        ExitText = 'Too many active constraints at the starting point.';
    case {Inform > 100}
...
% MODIFICATION LOG:
%
% 090726 bjo Created NLPL1 interface.
