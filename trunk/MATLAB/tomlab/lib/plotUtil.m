% plotUtil.m
%
% Utility to make plots in TOMLAB
%
% function  plotUtil(task, plotData, Runs, Solver, varargin)
%
% Input:
%    task      What to do:
...
% Copyright (c) 1990-2006 by Tomlab Optimization Inc., $Release: 5.5.0$
% Written May 31, 1990.    Last modified Aug 14, 2006.
  ...
  if task == 1
   ...
   if task==17 | task == 18
      ...
      switch Solver
         ...
         case {'rbfSolve','ego','arbfmip'}
            glbFile = 'cgoSave.mat';
         case 'glcDirect'
            glbFile = 'glcDirectSave.mat';
         case 'glbDirect'
            glbFile = 'glbDirectSave.mat';
         otherwise
            glbFile = '';
      end
      ...
      if exist(glbFile,'file')
         ...
         if strcmpi(Name1,deblank(Name))
            if strcmpi(glbFile,'cgoSave.mat')
               load(glbFile,'O')
               load(glbFile,'O', 'nInit')
               plot(O(ix(1),1:nInit),O(ix(2),1:nInit),'xm');
               plot(O(ix(1),nInit+1:end),O(ix(2),nInit+1:end),'.b');
            else
               load(glbFile,'C')
...
% Copyright (c) 1990-2004 by Tomlab Optimization Inc., Sweden. $Release: 4.0.0$
% Written Sept 18, 1995.   Last modified Nov 28, 1998.
%
...
% Kenneth Holmstrom, Tomlab Optimization Inc., E-mail: hkh@tomopt.com.
% Copyright (c) 1990-2004 by Tomlab Optimization Inc., Sweden. $Release: 4.0.0$
% Written Feb 11, 1998.    Last modified May 30, 1998.
%
...
if ~any(probType ==[checkType('mip'),checkType('miqp'),checkType('miqq')])
   if isempty(tM)
...
% 020423  hkh  Plot nInit first cgoSave points with other symbol.
% 020702  hkh  Use strcmpi, not strcmp, for Name check
% 030118  hkh  Change name to Tfzero, from dfzero
% 050520  ango Add glcDirect, glbDirect file names
% 050602  hkh  Test on miqp,miqq, not just mip
