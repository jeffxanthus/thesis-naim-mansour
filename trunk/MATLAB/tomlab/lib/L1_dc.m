% L1_dc.m
%
% function dc=L1_dc(x, Prob, varargin)
%
% L1_dc computes the gradient of the constraints c at the point x
% and the Jacobian matrix for the L1 residuals r(x).
% r is the residuals in the original formulation: min sum |r_i(x)|
% The extra variables have derivatives +1 and -1 exactly
...
if isempty(dc)
   dc=sparse([J,speye(m,m),-speye(m,m)]);
else
   dc=sparse([dc,zeros(size(dc,1),2*m);[J,speye(m,m),-speye(m,m)]]);
end
