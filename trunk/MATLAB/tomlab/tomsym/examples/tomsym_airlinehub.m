...
% flows should be split into a tensor of rank 4 with the indices
...
distances = tomArray([   0   945   605   4667   4749    4394;...
...
flows = tom('flows',cities^4,1,'int');
flows = tomArray(flows,[cities,cities,cities,cities]);
hub  = tom('hub', cities, 1, 'int');

% All variables are binary
bnds = {0 <= flows <= 1, 0 <= hub <= 1};

% Hub constraint
con1 = {sum(hub) == hubs};

% i and j constraint
con2 = {sum(sum(flows(i,j,k,l),k),l) == 1};

% Hub constraint 1 and 2
con3 = cell(cities,1);
con4 = cell(cities,1);
for iter = 1:cities
    con3{iter} = {flows(i,j,iter,l) <= hub(iter)};
    con4{iter} = {flows(i,j,k,iter) <= hub(iter)};
end

% Objective
objective = sum(vec(frights(i,j).*...
    (distances(i,k)+distances(l,j)+distances(k,l)*.8).*...
    flows(i,j,k,l)));
...
if PriLev > 0
    ...
    temp    = reshape(sol.flows(1:c^4),c,c,c,c);
    hubs    = find(sol.hub);
